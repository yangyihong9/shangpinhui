    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
              ...
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeOrder(1)">
                  <a>综合<span v-show="isOne" class="iconfont"
                      :class="{ 'icon-shang--jiantou': isAsc, 'icon-xia--jiantou': isDesc }"></span></a>
                </li>
          ...
          <div class="fr page">
            <!-- 
              total -- 整个分页器一共有多少条数据（只多不少） 。  pageSize -- 表示每一页需要展示多少条数据    pageNo -- 代表当前页数。
              continue -- 连续页码数(奇数,对称)
            -->
            <!-- 测试阶段，数据是假的 -->
...
export default {
  ...
  beforeMount() {
    ...
    // Object.assign --- ES6 新增， Object.assign(target, ...sources)
    // 可以合并对象 如果目标对象(target)与源对象(sources)具有相同的 key，则目标对象中的属性将被源对象中的属性覆盖，
  ...
  mounted() {
    // mounted 只会执行一次，但是搜索引用远不止一次，所以应该把 下面 封装成一个函数，函数可以尽享多次调用
  ...
  // }
  computed: {
    // mapGetters 里面的写法：传递的是数组，因为 getters 计算是没有划分模块【home，search】
  ...
  methods: {
    ...
    // 把这次的请求封装成一个函数，当你在需要的时候可以调用
    getData() {
      this.$store.dispatch('getSearchList', this.searchParams)
    },
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的数据置空，还需要向服务器发请求
      // 带给服务器的说明都是可有可无的（N)：如果属性值为空的字符串还是会把相应的字段带给服务器
      // 但是你把相应的字段变为 undefined ，当前这个字段不会带给服务器，方便
    ...
    // 点击排序事件（总和，价格）
    changeOrder(flag) {
      // flag 形参：它是一个标记，代表用户点击的是综合（1）价格（2）【用户点击的时候传递过来的】
      // 现获取 order 初始状态【咱们需要通过初始状态去判断接下来做什么】
      ...
      // 准备一个心得 order 属性值
      let newOrder = ''
      // 因为默认flag就是综合，所以当点击的flag 是综合，则代表点击了综合
      if (flag == originFlag) {
        // 默认综合是降序 desc ,如果点击的是综合（desc)，name箭头改变（asc),否则不变
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起 ajax 请求,获取相应搜索数据
      this.getData()
      // 每一次请求完毕，应该把相应的 1、2、3级分类的 id 置空，让她接收下一次相应的 1、2、3
      // 分类名字和关键字不用清理，因为每次路由发生变化的时候，都会给相应的数据
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
