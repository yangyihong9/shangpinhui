...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      // 目前 disNum形参 ： + 变化量（1）， - 变化量（-1）  impute最终的个数（并不是变化量）
      // type：为了区分三个元素
      // disNum形参：+变化量（1），-变化量（-1） impute最终的个数（并不是变化量）
      // cart :哪一个产品【身上有ID】
      // console.log('派发 action，通知服务器修改个数', type, disNum, cart);
      switch (type) {
        // 加号
        case 'add':
          // 带给服务器变化量
        ...
        case 'minus':
          ...
          break;
        case 'change':
          // 如果用户输入进来的最终量，非法的（带有汉字的|负数），带给服务器数字
