        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!--   以前的路由跳转：从 A 路由跳转到 B 路由，这里在加入购物车之前，进行路由跳转之前，发送请求，
                     把你购买的产品的信息通过请求的警示通知服务器，服务器进行相应的存储  -->
...
export default {
  ...
  // 当路由跳转到 /detail, 也就是路由挂载完毕的时候，执行 action
  mounted() {
    // 派发 action 获取产品详情信息
  ...
  computed: {
    ...
    // 给子组件的数据
    skuImageList() {
      // this.skuInfo.skuImageList 是 返回的Undefined，但是他至少得是一个数组 []
  ...
  methods: {
    ...
    changeActive(saleAttrValue, arr) {
      // 遍历全部售卖属性值 isChecked 为 0 就没有高亮了
      arr.forEach(item => {
        item.isChecked = 0
      })
    ...
    changeSkuNum(event) {
      ...
      // 用户输入进来的 文本*1 => 如果是数字还是数字，如果有字母或文字会是 NaN
      let value = event.target.value * 1
      // 如果用户输入进来的是非法的,出现 NaN 或者 负数
    ...
    async addShopCart() {
      ...
      /*
         当前这里是派发一个 action ，也想服务器发请求，判断加入购物车是成功还是失败了，进行相应的操作。
               this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
               上面这行代码是：调用仓库中的 addOrUpdateShopCart ，这个方法加上 async ，返回的一定是一个 Promise
                就是 要么成功 || 要么失败
      */
      //  try-catch主要用于进行异常处理,防止因为错误而造成系统崩溃
